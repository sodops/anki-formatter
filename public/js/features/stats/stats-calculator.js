...
export function calculateAndRenderStats() {
    try {
        ...
        decks.forEach(deck => {
            ...
            totalCards += count;
            cardsPerDeck.push({ name: deck.name, count: count, color: deck.color || '#7C5CFC' });
...
function renderMaturityChart(decks, containerId) {
    ...
    const segments = [
        ...
        { label: 'Mature', count: matureCount, color: '#7C5CFC' }
    ];
        ...
        return `<tr>
            <td style="padding:8px 12px;">
                <div style="display:flex;align-items:center;gap:8px;">
                    <span style="width:10px;height:10px;border-radius:50%;background:${deck.color || '#7C5CFC'};display:inline-block;"></span>
