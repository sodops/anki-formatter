...
> **Uzbek version available**: See [CHANGELOG.md](./CHANGELOG.md) for Uzbek documentation.
...
- Turbopack dev server (faster build)
...
- Fixed `auto-focus` issue for buttons in mobile mode
...
- Fixed drag-drop card ordering — via store dispatch, correct index calculation

**New Features:**

- Suspend cards (suspend/bury) — temporarily remove cards from learning
...
- localStorage quota management — error handling when memory full
...
- Google Docs import — get cards via URL
- Study Mode — flashcard learning session with flip animation
